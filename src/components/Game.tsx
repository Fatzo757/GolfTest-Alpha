...
export default function Game({ gameId, token, user, onExit, onRematch }: GameProps) {
  ...
  const turnIndicator = (color: string) => (
    <motion.div 
      ...
      className={`px-2 md:px-3 py-1 flex items-center h-full gap-1.5 ${color} shrink-0`}
    >
      <motion.div
        animate={{ x: [0, 4, 0] }}
        ...
        <div className="w-full flex flex-col items-center justify-center min-h-[140px] md:h-[160px] lg:h-auto">
          <AnimatePresence mode="wait" initial={false}>
            {state?.game?.drawn_card ? (
              ...
              >
                <span className="text-[10px] md:text-xs text-ui-yellow font-black uppercase tracking-[0.2em] animate-pulse absolute -top-5 md:-top-6 whitespace-nowrap">Active</span>
                <motion.div 
                  animate={{ y: [0, -6, 0] }} 
                ...
                <div className="pointer-events-auto flex flex-1 ml-4 items-center justify-between gap-2 md:gap-4 bg-bg-dark/40 backdrop-blur-md px-3 py-2 md:px-5 md:py-2.5 border border-ui-border rounded-xl shadow-lg">
                  ...
                  <div className="flex items-center gap-4 md:gap-5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[1.25rem] md:text-[1rem] text-ui-green font-black">{userId === state.game.player1_id ? state.game.player1_total_score : state.game.player2_total_score}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[1.25rem] md:text-[1rem] text-ui-red font-black">{userId === state.game.player1_id ? state.game.player2_total_score : state.game.player1_total_score}</span>
                    </div>
                  </div>
                       ...
                       <div className="absolute top-0 -translate-y-1/2 left-4 md:left-6 max-w-[calc(100%-2rem)] md:max-w-[calc(100%-3rem)] bg-bg-dark tracking-widest uppercase flex items-stretch overflow-hidden border-2 border-ui-red z-10">
                          <div className="px-2 md:px-3 py-1 md:py-1.5 flex flex-col justify-center min-w-0">
                            <div className="flex items-center gap-1.5 text-ui-red">
                               <UserAvatar type={(state.game as any).player2_avatar} size={14} className="hidden md:block shrink-0" />
                               <UserAvatar type={(state.game as any).player2_avatar} size={10} className="md:hidden shrink-0" />
                               <span className="text-[0.65rem] sm:text-xs md:text-sm lg:text-base font-bold truncate block max-w-[90px] sm:max-w-[130px] md:max-w-[180px] lg:max-w-[220px]">{opponentName || 'OPPONENT'}</span>
                            </div>
                            <div className="text-[0.55rem] sm:text-[0.65rem] md:text-xs text-white/70 font-black mt-0.5">
                              {calculateScore(opponentId || 'cpu')} PTS
                            </div>
                         </div>
                         {!isMyTurn && state.game.status === 'playing' && (
                           <div className="shrink-0 flex items-center border-l border-white/10 bg-ui-red/10">
                             {turnIndicator('text-ui-orange')}
                           </div>
                         )}
                       </div>
                       <motion.div 
                       ...
                       <div className="absolute top-0 -translate-y-1/2 left-4 md:left-6 max-w-[calc(100%-2rem)] md:max-w-[calc(100%-3rem)] bg-bg-dark tracking-widest uppercase flex items-stretch overflow-hidden border-2 border-ui-green z-10">
                         <div className="px-2 md:px-3 py-1 md:py-1.5 flex flex-col justify-center min-w-0">
                            <div className="flex items-center gap-1.5 text-ui-green">
                               ...
                               <span className="text-[0.65rem] sm:text-xs md:text-sm lg:text-base font-bold truncate block max-w-[90px] sm:max-w-[130px] md:max-w-[180px] lg:max-w-[220px]">{myName}</span>
                            </div>
                            <div className="text-[0.55rem] sm:text-[0.65rem] md:text-xs text-white/70 font-black mt-0.5">
                         ...
                         {isMyTurn && state.game.status === 'playing' && (
                           <div className="shrink-0 flex items-center border-l border-white/10 bg-ui-green/10">
                             {turnIndicator('text-ui-yellow')}
                           </div>
                         )}
                       </div>
                           ...
                           >
                             ...
                             {state.game.status === 'initializing' && !card.is_face_up && (
                               <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                 <span className="text-xs text-ui-yellow font-bold opacity-0 group-hover:opacity-100 uppercase tracking-widest bg-bg-dark/80 px-2 py-1 border border-ui-yellow transition-opacity">Reveal</span>
                               </div>
                             )}
                                        ...
                                        try {
                                          ...
                                          const renderMiniGrid = (gridCards: any[], label: string) => (
                                            <div className="flex flex-col items-center">
                                              <span className="text-xs text-ui-gray mb-1 uppercase">{label}</span>
                                    ...
                                    <>
                                      ...
                                      <div className="flex items-center gap-3 mt-1">
                                        ...
                                        <div className="flex flex-col gap-0.5 min-w-0">
                                          ...
                                          <span className="text-xs text-white/80 uppercase truncate">Pos: {move.card_affected_index}</span>
                                        </div>
                                      </div>
                                    </>
                                  )}
               ...
               <div className="grid md:grid-cols-2 gap-12 relative z-10">
                 ...
                 >
                    ...
                    <div className="flex justify-between items-end mt-auto">
                      <span className="text-xs text-ui-gray uppercase">Round Total</span>
                      <motion.span 
                        initial={{ scale: 1 }}
          ...
          >
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 py-6 relative z-10">
                ...
                >
                  ...
                  <div className="flex flex-col relative z-10">
                    ...
                    <span className="text-xs text-ui-gray uppercase font-bold tracking-tighter">Total Points</span>
                  </div>
                  {state.game.winner_player_id === state.game.player1_id && (
                    <motion.div 
                      initial={{ scale: 0 }}
                      ...
                      className="absolute top-2 right-2 bg-ui-yellow text-bg-dark text-xs font-black px-2 py-0.5 rotate-12 shadow-md z-20"
                    >
                      WINNER
                    </motion.div>
                  )}
              ...
              >
                ...
                <div className="text-xs text-ui-gray mt-2 tracking-[0.4em] font-bold uppercase opacity-50">
